...
/*
    Find longest path to climb stairs,
...
public class ClimbingStairs {
    ...
    public int longestPathUsingRecursion(int n, int currentStep) {
        if (currentStep == n) {
            return 0;
        }
        if (currentStep > n) {
        ...
        int choice1 = longestPathUsingRecursion(n, currentStep + 1);
        int choice2 = longestPathUsingRecursion(n, currentStep + 2);
        return 1 + Math.max(choice1, choice2);
    }

    public int longestPathUsingMemoization(int n, int currentStep, Map<Integer, Integer> lookupTable) {
        ...
        int choice1 = longestPathUsingMemoization(n, currentStep + 1, lookupTable);
        int choice2 = longestPathUsingMemoization(n, currentStep + 2, lookupTable);
        int max = 1 + Math.max(choice1, choice2);
        lookupTable.put(currentStep, max);
        return max;
    }

    public int longestPathUsingTabulation(int n) {
        int[] dp = new int[n];
        dp[0] = 1;
        dp[1] = 2;
        for (int i = 2; i < n; i++) {
    ...
    public int longestPathUsingOptimizedTabulation(int n) {
        int prev = 1;
        int curr = 2;
        int next = curr;
        for (int i = 2; i < n; i++) {
            next = 1 + Math.max(prev, curr);
            prev = curr;
            curr = next;
        }
        return next;
